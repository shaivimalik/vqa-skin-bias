...
model.load_adapter("microsoft/Phi-4-multimodal-instruct", adapter_name="vision", device_map="cuda:0", adapter_kwargs={"subfolder": 'vision-lora'}) 
...
for trait in trait_words:
    ...
    for img_file in img_paths:
        ...
        for idx, template in enumerate(question_templates):
            ...
            prompt = f'{user_prompt}<|image_1|>Instructions: Analyze the provided image and answer the following question with exactly one of these responses: Yes or No.\
                                                Question: {question}\
                                                Response format: Yes or No{prompt_suffix}{assistant_prompt}'
    ...
    res_df_final.to_csv(os.path.join(args.output_dir, f'Phi-4-multimodal-instruct_{trait}.csv'), index=False)